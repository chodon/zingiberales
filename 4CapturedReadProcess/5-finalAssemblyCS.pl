...
-b  INT    min length for a contig to be kept [80]
...
-p  INT    the overlap identity in cap3 [90]
\n\n/) unless (@ARGV);


my %opts = (a=>undef, b=>80, c=>4000, d=>0.99, e=>0.95, f=>"all", p=> 90);
getopts('a:b:c:d:e:f:p:', \%opts);
...
sub clusterBlat {
	...
	foreach my $cluster (keys %clusters) {
		...
		my $call = system("cap3 " . $temp . " -z 1 -o 16 -e 11 -s 251 -p $c3p");
